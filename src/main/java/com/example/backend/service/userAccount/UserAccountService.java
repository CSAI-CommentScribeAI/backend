...
public class UserAccountService {
    ...
    public UserAccountResponseDTO getMyInfoBySecurity() {
       UserAccount userAccount = userAccountRepository.findByUserId(String.valueOf(SecurityUtil.getCurrentUserId()))
    ...
    public UserAccountResponseDTO changeUserAccountPassword(Authentication authentication, String password, String newPassword) {
        ...
        UserAccount userAccount = userAccountRepository.findByUserId(authUser.getUserId())
                .orElseThrow(() -> new RuntimeException("유저 정보를 찾을 수 없습니다."));
        UserAddress userAddress = userAddressRepository.findById((long) userAccount.getAddress()).isEmpty()
                ? null : userAddressRepository.findById((long) userAccount.getAddress()).get();
        return UserAccountResponseDTO.of(userAccountRepository.save(userAccount), userAddress);
